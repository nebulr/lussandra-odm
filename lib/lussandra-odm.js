...
Lussandra.prototype.sync = function (Instance) {
    ...
    this._connection.execute(describeVersion, [])
    .then(function(result) {
      var version = result[0].replace('-SNAPSHOT', '').split('.');
      var describeTablesQuery = "select * from system_schema.columns where keyspace_name=? and table_name=?;";
      if (version[0] === '2') {
        describeTablesQuery = "SELECT * FROM system.schema_columnfamilies WHERE keyspace_name=? AND columnfamily_name =?";
      }
      var describeTablesArguments = [self._connectionOptions.keyspace.toLowerCase(), model._internalState.m_tableName];
      return self._connection.execute(describeTablesQuery, describeTablesArguments);
    })
    .then(function(result) {
