...
module.exports = function(instance, opts) {
  ...
  function getSelectParameters(obj, schema, criteria) {
    ...
    for (var key in criteria) {
      ...
      var property = { name : key, value : schema[key].type.toCQLString(criteria[key]), comparer : comparers.EQUALS };
  ...
  function find (instance, params, criteria) {
    ...
    return new Promise (function (resolve, reject) {
        ...
        var sql = 'select * from [[table_name]] where [[keys]] allow filtering;';

        if(self.criteria.hasOwnProperty('limit')) {
          sql = sql.replace (';', ' limit ' + self.criteria.limit + ';');
        }

        if(self.criteria.hasOwnProperty('aggregate')) {
          ...
          if (self.criteria.hasOwnProperty('fields') && self.criteria.fields.length === 1) {
            sql = sql.replace ('*', self.criteria.fields[0]);
          }
        } else if (self.criteria.hasOwnProperty('fields')) {
