...
module.exports = function(instance, opts) {
  ...
  function getSelectParameters(obj, schema, criteria) {
    ...
    for (var key in criteria) {
      ...
      if (typeof schema[key] === 'object' && schema[key].type.type_name !== 'JSON to Text' && schema[key].type.type_name !== 'MAP'
      ...
      } else if (typeof schema[key] === 'object' && schema[key].type.type_name !== 'JSON to Text' && schema[key].type.type_name !== 'MAP'
          && criteria[key].hasOwnProperty('AND')) {
        for (var i = 0; i < criteria[key]['AND'].length; i++) {
          property = { name : key, value : schema[key].type.toCQLString(criteria[key]['AND'][i].value), comparer : criteria[key]['AND'][i].comparer };
          writeSelectKey (property, schema, keys, values);
        }
      } else {
        // Default equals
