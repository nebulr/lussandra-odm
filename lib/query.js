...
module.exports = function(instance, opts) {
  ...
  function update (instance, updateParams) {
    return new Promise (function (resolve, reject) {
        ...
        try {
            var updateKeys = getSelectParameters(instance, instance._schema, updateParams);
        } catch (err) {
  ...
  function insert (instance, params) {
      return new Promise (function( resolve, reject ) {
        ...
        params = getInsertParameters(instance, instance._schema);
        sql = sql.replace('[[table_name]]', query.tableName);
        sql = sql.replace('[[keys]]', params.keys);
  ...
  function findDetailed (instance, params, criteria) {
    return new Promise (function (resolve, reject) {
        ...
        if(criteria.limit != null) {
          sql = sql.replace (';', ' limit ' + criteria.limit + ';');
        }
        // Aggregate is priority
        if(criteria.aggregate != null) {
          if (criteria.aggregate === 'count') {
            sql = sql.replace ('*', 'count(*)');
          } else if (criteria.aggregate === 'sum') {
            sql = sql.replace ('*', 'sum(*)');
          }
        // Omit is secondary
        } else if (criteria.omit != null && criteria.omit.length > 0) {
          // Turn the array of omitted fields into a hashmap
          var omissions = {};
          for (var i = 0; i < criteria.omit.length; i++) {
            omissions[criteria.omit[i]] = true;
          }
        ...
        // Fields takes last
        } else if (criteria.fields != null && criteria.fields.length > 0) {
          sql = sql.replace ('*', criteria.fields.join());
        }

        if (criteria.pageState) {
            query.options.pageState = criteria.pageState;
        }

        if (criteria.fetchSize) {
          query.options.fetchSize = criteria.fetchSize;
        }
