...
Instance.prototype._compile =  function(_schema, lussandra, opts, state, _model) {
  var model = function (properties) {
    this._internalState = {
      m_lussandra: lussandra,
      m_state: state || state.UNKNOWN,
      m_oldState: state || state.UNKNOWN,
      m_schema: _schema,
      m_isDirty: false,
      m_opts: opts,
      m_tableName: opts.tableName || state.UNKNOWN,
      m_dirtyFields: [],
      m_preActions: [],
      m_postActions: [],
      m_model : _model
    ...
    for (var key in properties) {
        ...
        this._internalState.m_dirtyFields.push(key);
        this._doc[key] = properties[key];
    }
  ...
  return model;//new model(_schema, lussandra, opts, state, _model);
};
...
Instance.prototype._init = function() {
  ...
  for (var property in this._internalState.m_schema) {
    ...
    if (!states.equals(this._internalState.m_state, states.NEW) ) {
...
Instance.prototype._stubFunc = function(prop){
  return {
    ...
    set: function (value) {
      ...
      // Comparing objects is shotty; Assume they changed value
      if (!equal(this._doc[prop], theVal) ||
          typeof this._doc[prop] === 'object' &&
          typeof theVal === 'object') {
        this._internalState.m_isDirty = true;
...
Object.defineProperty(Instance.prototype, "_client", {
  get: function myProperty() {
    return this._internalState.m_lussandra.client;
  }
});
...
Instance.prototype.insert = function(query) {
  return this._internalState.m_query.insert(this, query);
};

Instance.prototype.update = function(query) {
  return this._internalState.m_query.update(this, query);
};

Instance.prototype.findDetailed = function(query, criteria) {
    var _criteria = (criteria) ? criteria : {};
    return this._internalState.m_query.findDetailed(this, query, _criteria);
};

Instance.prototype.find = function(query, criteria) {
  var _criteria = (criteria) ? criteria : {};
  return this._internalState.m_query.find(this, query, _criteria);
};

Instance.prototype.findOne = function(query, criteria) {
  var _criteria = (criteria) ? criteria : {};
  return this._internalState.m_query.findOne(this, query, _criteria);
};

Instance.prototype.findLucene = function (query, critera) {
  var _criteria = (criteria) ? criteria : {};
  return this._internalState.m_query.findLucene (this, query, _criteria);
}

Instance.prototype.remove = function(query) {
  return this._internalState.m_query.remove(this, query);
};
