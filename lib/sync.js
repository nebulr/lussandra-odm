        ...
        if (options.replication) {
            if (options.replication.strategy === 'SimpleStrategy') {
                ...
                if (options.replication.strategy_options) {
                    ...
                    replicationStrategy = JSON.stringify(replicationStrategy).replace(/"/g, '\'');
                } else {
...
exports.updateTable = function (model, cql) {
  ...
  cql._connection.execute(cql.describeTablesQuery, describeTableArguments)
  .then(function(result) {
      ...
      for (var key in schema)  {
          checklist[key.toLowerCase()] = false;
      }
      ...
      for (var i = 0; i < result.rows.length; i++) {
          ...
          // This is different based on version of cassandra
          if (row.key_aliases) {
            column_name = JSON.parse(row.key_aliases)[0];
          } else if(row.column_name) {
            column_name = row.column_name;
          } else if(row.columnfamily_name) {
            column_name = columnfamily_name;
          }

          checklist[column_name] = true;
      }
