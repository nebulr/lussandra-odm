...
exports.checkKeyspaceExists = function (cql, keyspace, options) {
  ...
  return new Promise (function (resolve, reject) {
    ...
    .then(function(results) {
      var newSelf = self;
      self.cql.shutdown(function() {
          newSelf.resolve(results);
      });
    })
    .catch (self.reject);
  });
      ...
      .then(function(result) {
          ...
          for (var key in schema) {
              ...
              for (var i = 0; i < result.rows.length; i++) {
                  var column_name = result.rows[i].column_name || result.rows[i].columnfamily_name;
                  if (column_name.toLowerCase() === key.toLowerCase()) {
                      found = true;
                      break;
                  }
              }
